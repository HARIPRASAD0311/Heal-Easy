...
export default function PatientInfoCard({ patientId, onNameResolved }) {
  ...
  // Bubble the patient name up to the header once resolved
  if (patient?.name && onNameResolved) onNameResolved(patient.name);

  if (loading) return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
      <Spinner message="Loading patient information…" />
    </div>
  );
  ...
  if (!patient) return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
      <EmptyState
        ...
        subtitle="Your profile hasn't been set up yet."
        action={{ label: "Go to registration", onClick: () => window.location.href = "/register" }}
      />
    </div>
  );
