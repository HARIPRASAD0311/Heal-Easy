...
export default function Queue() {
  ...
  const { data: queue, loading, error, refetch } = useFetch(fetchQueue, [patientId], { skip: !patientId });
      ...
      <main className="max-w-md mx-auto px-4 py-8 space-y-5">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Live position in your department queue.</p>
          </div>
          <button
        ...
        {loading ? (
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            <Spinner message="Fetching queue status…" />
          </div>
        ) : error ? (
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          ...
          <>
            ...
            <div className={`rounded-2xl shadow-sm border overflow-hidden
              ${isNext ? "border-teal-200 bg-teal-50" : "bg-white border-slate-100"}`}>
              ...
              <div className="px-5 py-5 grid grid-cols-3 gap-4">
                ...
                <div className="flex flex-col items-center">
                  <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-2 shadow-sm
                    ${isNext
                ...
                <div className="flex flex-col items-center">
                  <div className="w-16 h-16 rounded-2xl bg-blue-50 flex items-center justify-center mb-2">
                    <span className="font-black text-2xl text-blue-600 tabular-nums">
                      #{queue.position ?? "—"}
                    </span>
                  </div>
                ...
                <div className="flex flex-col items-center">
                  <div className="w-16 h-16 rounded-2xl bg-amber-50 flex items-center justify-center mb-2">
                    <div className="text-center">
                      <p className="font-black text-lg text-amber-600 tabular-nums leading-none">
                        {queue.estimatedWait ?? "—"}
                      </p>
                      {queue.estimatedWait && (
                        <p className="text-[9px] text-amber-500 font-bold uppercase tracking-wide mt-0.5">min</p>
                      )}
                    </div>
                  </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 px-5 py-4 flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-teal-100 flex items-center justify-center flex-shrink-0">
                  <span className="text-teal-700 font-bold text-base">
                    {queue.doctorName.charAt(0).toUpperCase()}
                  </span>
                </div>
                <div>
                  ...
                  {queue.doctorSpecialty && (
                    <p className="text-xs text-slate-500">{queue.doctorSpecialty}</p>
                  )}
                </div>
              </div>
            )}
